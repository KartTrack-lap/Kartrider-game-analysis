...
$$
트랙 사용수 = 36650 + 1102.6932×자연테마   -3206.6543×log(평균주행시간) + 463.1283×난이도
$$
...
$$
\begin{aligned}
    `평균 주행 시간` = -61960 + 13470×`펜스없는구간유무` + 1761.5126×`내리막길 비율`  \\
    + 2603.9692×`예각커브개수` + 5346.4972×`헤어핀개수` \\
    + 1654.3477×`둔각커브개수` + 116400×`직선비율` + 941.0335×`고정장애물개수`
\end{aligned}
$$
...
$$
\begin{aligned}    
    `난이도` = 1.0359 +  0.3431×펜스없는구간유무 + 0.0341× 내리막길개수×lap -0.0076×전체직선개수×lap \\
    + 0.1082×트랙이동개수× lap + 0.0381× 감속트리거개수×lap + 0.1276 × 점프트리거개수×lap \\    
    + 0.0737×헤어핀구간개수×lap + 0.0299× 전체곡선구간개수×lap + 0.0223×전체장애물개수×lap
\end{aligned}
$$
